...
app.get("/api/models", async (req, res) => {
  try {
    ...
    res
      .status(502)
      .json({ error: "Cannot reach Prism app. Make sure it is running." });
  }
});
...
wss.on("connection", (ws) => {
  ...
  ws.send(JSON.stringify({ type: "terminalHistory", lines: terminalBuffer }));
...
async function handleCommand(ws, msg) {
  ...
  switch (action) {
    ...
    case "scroll": {
      const result = await activeEngine.scroll(
    ...
    case "clearHistory": {
      conversationHistory = [];
      ws.send(
        JSON.stringify({ type: "result", action, data: { status: "cleared" } }),
      );
      break;
    }
    ...
    default:
      ws.send(
        JSON.stringify({ type: "error", text: `Unknown action: ${action}` }),
      );
  }
}
...
async function handleChat(ws, payload) {
  ...
  try {
    ...
    ws.send(
      JSON.stringify({ type: "error", text: `Chat error: ${err.message}` }),
    );
  }
}

function pushTerminalLine(level, parts) {
  const text = parts
    .map((part) => {
  ...
  if (terminalBuffer.length > MAX_TERMINAL_LINES) {
    terminalBuffer.splice(0, terminalBuffer.length - MAX_TERMINAL_LINES);
  }

  const payload = JSON.stringify({ type: "terminal", entry });
...
console.log = (...args) => {
  ...
  pushTerminalLine("log", args);
};

console.error = (...args) => {
  originalConsoleError(...args);
  pushTerminalLine("error", args);
};

console.warn = (...args) => {
  originalConsoleWarn(...args);
  pushTerminalLine("warn", args);
};
...
async function handleAgentRun(ws, payload) {
  ...
  if (!activeEngine || !activeEngine.isOpen()) {
    return ws.send(
      JSON.stringify({ type: "error", text: "Launch a browser first" }),
    );
  }
  ...
  agentHistory.push({
    role: "assistant",
    content:
      "Understood. I will always include a JSON action block in every response. Ready for the task.",
  });
  ...
  while (isAgentRunning) {
    ...
    try {
      ...
      ws.send(
        JSON.stringify({ type: "error", text: `Agent error: ${err.message}` }),
      );
      break;
    }
    ...
    if (!agentAction) {
      ...
      try {
        ...
        ws.send(
          JSON.stringify({
            type: "error",
            text: `Agent retry error: ${err.message}`,
          }),
        );
        break;
      }
      ...
      console.log(
        "[Agent] Retry response (first 500):",
    ...
    if (!agentAction) {
      ws.send(
        JSON.stringify({
          type: "agentStep",
          step: stepCount,
          text: "No action found in response. Stopping.",
        }),
      );
      break;
    }
    ...
    if (agentAction.action === "done") {
      ws.send(
        JSON.stringify({
          type: "agentDone",
          summary: agentAction.summary || "Task complete",
        }),
      );
      break;
    }
    ...
    try {
      ...
      ws.send(
        JSON.stringify({
          type: "agentStep",
          step: stepCount,
          text: "Browser closed or page error.",
        }),
      );
      break;
    }
...
async function executeAgentAction(action) {
  switch (action.action) {
    case "navigate":
      return activeEngine.navigate(action.url);
    case "click":
      return activeEngine.click(action.selector);
    case "type":
      return activeEngine.type(action.selector, action.text);
    case "pressKey":
      return activeEngine.pressKey(action.key);
    case "scroll":
      return activeEngine.scroll(
        action.direction || "down",
        action.amount || 400,
      );
    case "newTab":
      return activeEngine.newTab(action.url);
    case "switchTab":
      return activeEngine.switchTab(action.index);
    default:
